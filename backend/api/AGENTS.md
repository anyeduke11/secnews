...
> 架构数字(`70 routers`)由 `scripts/generate_meta.py` AST 反推维护,不要手改。

## 子树身份

FastAPI 路由层(70 routers,`include_router` 注册入口),**唯一对外 HTTP 入口**。
...
- **Owner 模块**: 新增/修改路由后必须跑:
  ...
  python -m pytest backend/tests/ --tb=short -q                             # 全量回归
   python scripts/generate_meta.py --check                                    # CI 校验 70 routers 计数
  ```
