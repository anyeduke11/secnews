...
## §17 dsh 融合架构图（2026-08-21 补 §13 视觉化）

> §13 / §16 给的是文字协议和组件说明；本节给 **3 张 ASCII 架构图**直接当开发参照：
> §17.1 cordis.yml hotspot 派生版, §17.2 dsh 状态机, §17.3 M4 部署 8 步时序。
...
**T15a — dsh 主进程接入**（即原 T15，见 §16/§17，不变）

**T15b — 外部 CLI runner 注册**：
1. dsh 配置（cordis.yml `agents` 段）注册两个自定义 runner：
   ...
   agents:
     ...
     codex:
       ...
       protocol: jsonl
   ```
2. `agent_bridge.py` 增加 `route(task_type)` — 按 19.2 表选 runner；
   AIView 发送消息时携带 `preferred_agent` 字段
3. 工作目录安全边界：claude/codex 只允许在 `codegarden/<project>/` 内执行
   （dsh runner `cwd` 锁定 + sandbox flag），禁止触碰 hotspot 根目录与 knowledge/
