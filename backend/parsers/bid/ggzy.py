"""全国公共资源交易平台 (ggzy.gov.cn) 解析器。

站点结构:
- 列表页: https://www.ggzy.gov.cn/info/zbgg
...
class GgzyParser(BaseSourceParser):
    """全国公共资源交易平台解析器。"""
    ...
    def _do_parse(self, content: str, url: str, content_type: str = "html") -> list[RawItem]:
        ...
        for m in _ITEM_RE.finditer(content):
            ...
            href = raw_url if raw_url.startswith("http") else f"https://www.ggzy.gov.cn{raw_url}"
            if href in seen:
                continue
            seen.add(href)
