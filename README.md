...
<p align="center">
  <img src="./assets/readme/hero.svg" width="100%" alt="SecNews v0.8.1 — 单人本地 AI + 安全工作站: Sentinel Terminal 三层工作流 (Data/Judge/Action) + 8 子系统 + 19 MCP 工具">
</p>

> **SecNews**（开发代号 `hotspot`）是面向 **AI + 安全从业者** 的单人本地工作站。
> 全站统一收敛为 **Sentinel Terminal** 三层工作流：**Data → Judge → Action**，
...
> **重大变更**：原 `/workbench` 5 视图已**物理删除**并并入 `/secnews` 统一工作台；
> `dsh` 认知大脑从外部子进程降级为**受管子进程**（gate `dsh=true` 由前端一键启停）。
> v0.6.3 起 `knowledge/` 根被废弃，**唯一真源为 `llm-wiki-2.0/`** md 文件，SQLite 只做投影索引。
...
| 版本 | 发布日期 | 关键里程碑 | 源码归档 |
|------|----------|------------|----------|
| **v0.8.1** | 2026-09-05 | 七 gate 开闸 + 运行时弹性层 + graceful shutdown + deep 场景权重重排 | [Source code (zip)](https://github.com/anyeduke11/secnews/releases/tag/v0.8.1) |
| **v0.8.0** | 2026-09-04 | v0.8 Skills 四阶段收口：20 内置 skill + `/skill-store` + `/dashboard` | [Source code (zip)](https://github.com/anyeduke11/secnews/releases/tag/v0.8.0) |
| **v0.7.4** | 2026-09-01 | 观测深化 + 扩展开闸 + i18n 全量 + secrets ACL + 历史债清偿 | [Source code (zip)](https://github.com/anyeduke11/secnews/releases/tag/v0.7.4) |
