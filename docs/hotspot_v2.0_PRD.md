...
>
> **命名说明**: v1.8 已被 catchup 阶段占用，v1.9 已被 checkpoint/validation 占用。下一个"大版本"命名为 **v2.0**。
...
- [Part B · v2.0 PRD 新方案](#part-b--v20-prd-新方案)
  - [B.0 产品战略审计（2026-07-27 团队产出）](#b0-产品战略审计2026-07-27-团队产出)
    ...
    - [B.0.8 Non-goals（v2.0 明确不做什么）](#b08-non-goalsv20-明确不做什么)
  ...
  - [B.9 不做清单（v2.0 明确放弃）](#b9-不做清单v20-明确放弃)
...
- [附录 E · v2.0 PRD 实测对账与修复记录（2026-07-27 leader skill 产出）](#附录-e--v20-prd-实测对账与修复记录2026-07-27-leader-skill-产出)
- [附录 F · v2.0 PRD 修复任务书归档（2026-07-27，已完成）](#附录-f--v20-prd-修复任务书归档2026-07-27已完成)
...
| **应用了 `lifecycle` 字段的 items** | **4,147 / 4,147 = 100%** | 全部 items 应有生命周期 | ✅ 字段已建，但值命名与 v2.0 5 阶段不统一 |
| **应用 v2.0 5 阶段 (`kl:raw/refine/link/structure/publish`) 的 items** | **0 / 4,147 = 0%** | v2.0 起新写入 | ❌ **老数据全部用 v1.7 旧 3 阶段值**（见下）|
| **现存 lifecycle 实际取值分布** | `generate`: 74 / `signal`: 4,070 / `amplify:tagged`: 3 | v2.0 目标 5 阶段 | ❌ 与 v2.0 设计 100% 不一致，需 B.11.6 增加迁移 SQL |
...
2. **知识库 4,147 items 100% 有 lifecycle 但 0% 用 v2.0 5 阶段（实际 100% 用 v1.7 旧 3 阶段值 `generate`/`signal`/`amplify:tagged`）** —— KL 五阶段是"应该实现但被遗忘的脚手架"
...
### A.3.1 过度设计（应该砍或简化）— v2.0 适度裁剪

> **原则**: 砍掉 v1.7 中"未实施且无 KPI 价值"的设计；保留"未实施但价值明确"的设计为 v2.0 候选；保留"已实施且价值明确"的设计为必选。

| 设计 | 问题 | v2.0 处置 | 备注 |
|---|---|---|---|
| **KL 五阶段**（`kl:raw/refine/link/structure/publish`）| 实际 100% items 有 `lifecycle` 字段但 0% 用 v2.0 5 阶段（100% 用 v1.7 旧 3 阶段值 `generate`/`signal`/`amplify:tagged`）；5 阶段是知识复利的必要骨架 | **完全保留** + 加 5 个自动化触发器 + migration 046 强制命名迁移 | 不应简化 |
| **SAG 事件-实体模型**（`item_entities` 表 + chunks YAML 字段）| 实际未启用 chunks 字段；实体抽取依赖 LLM Agent，hotspot 本地无运行 | **完全保留**：`item_entities` 表 + chunks 字段 v2.0 引入（Hybrid AI 提供本地 LLM 支持）|
| **SAG 命名**（原 Signal-Amplify-Generate）| 与 Zleap-AI/SAG 命名冲突已改 KL，但文档/代码仍残留 | v2.0 全量清理引用 | |
| **kv_cache 表** | v1.7.6 评估后保留为可选，**但**实际无人维护 | **删除**（保留 schema 注释在 docs/）| |
| **6 种认知模式**（简报/扫描/深度/整理/复习/告警）| v1.7 5 个未实施；v2.0 凭借 Hybrid AI + chunks + attention_score 全部实施 | **v2.0 全部实施**（Phase 12 实施 4 种核心 + Phase 16 实施整理/复习；用户明确反对激进裁剪到 3 种）| 6 模式完整闭环 |
| **告警系统 M6** | 整套规则引擎设计未实施 | **保留设计 + 部分实施**：v2.0 实施 3 类基础规则（tech_stack 影响 / 关键 CVE / 标讯命中），v2.1 扩展 | 用户明确要求恢复 |
| **离线间隔摘要** | 设计但未实施 | **v2.0 实施**：catchup 失败恢复后自动生成"昨日未读补丁"摘要 | |
| **快速捕捉 Quick Capture** | 设计但未实施 | **推迟到 v2.2**（浏览器插件开发成本高）| |
| **注意力热图** | 设计但未实施 | **v2.0 引入**（基于 reading_states + LLM，详见 B.4.6）| |
| **决策日志** | 设计但未实施 | **不做**（概念模糊）| |
...
| **Phase 5 内部 agent 代码** | 已被 Phase 7 Option A 替代但代码未清干净 | v2.0 **必须清理 `/api/agent/*` deprecated 路由** | 落实 Option A |
| **`/api/agent/*` 路由** | CLAUDE.md 标记为 deprecated 但仍存在 | v2.0 移除 | 落实 Option A |

### A.3.2 缺失设计（v2.0 必须补）

| 缺失 | 原因 | v2.0 解决方案 |
|---|---|---|
| **复利机制无落地** | 知识库 4,147 items 100% 有 lifecycle 但 0% 用 v2.0 5 阶段、无法证明复利 | 见 B.8 复利机制 |
| **抓取层没有跨源去重** | Horizon 有 dedup，hotspot 没有 | v2.0 加 LSH/simhash 跨源去重 |
| **没有 AI 评分** | Horizon 0-10 AI 评分；hotspot 仅靠源信任度 | v2.0 用外部 AI Agent 通过 MCP 评分（保持零本地 LLM）|
| **没有背景补全** | Horizon "为陌生概念补充背景" | v2.0 用外部 AI Agent 调 MCP 触发，缓存在 concept.md |
| **没有评论/讨论捕获** | Horizon 抓 HN/Reddit 评论 | v2.0 不做（single-user 不需要社区评论）|
| **没有 daily digest 自动生成** | 用户每天 50-200 篇无摘要 | v2.0 用外部 AI Agent 每日生成 .md 日报，存到 `knowledge/summaries/daily/` |
| **Codegarden 与 knowledge 联动弱** | Codegarden 项目的 tech_stack 应自动从 knowledge 推 | v2.0 加 `tech_stack_drift` 任务：知识库新概念→触发项目技术栈重评估 |
| **Security Graph 与 knowledge 隔离** | CVE/ATT&CK 节点在 security 库，相同概念在 knowledge 库重复 | v2.0 统一 entity 命名空间，security 节点引用 knowledge concepts |

### A.3.3 必要性审视（哪些章节是必需的？）

> **v2.0 章节保留原则**：保留 v1.7 全部 17 章骨架，但**精简内容**而非删除章节；保留所有"反映 hotspot 设计灵魂"的章节（用户旅程、6 种认知模式、告警系统设计）。
...
**v1.7 PRD 共 2193 行 → v2.0 PRD 当前 4087 行（+86%；因补全 KL 5 阶段 + 5 触发器 + Hybrid AI + Chunks/Attention + 资讯收藏聚合视图 + 规划引导 + 6 张新表 + 附录 E/F 审计归档等模块；远期可精简到 2500-3000 行）**
...
| **daily-run.sh** | hotspot 已有 APScheduler → 但 v2.0 加 daily_digest job | 自动化日报 |
...
1. **P0**（v2.0 必做）：统一 `BackendSession` + 可读 ID + simhash 跨源去重
2. **P1**（v2.0 必做）：trafilatura 作为可选 extractor + AI 评分 MCP tool + 背景补全 MCP tool
3. **P2**（v2.0 必做）：JSON pipeline_config 化（最简版，4 个源示例）
...
- 46 源已建立，200-500 条/天
- 真正缺的是"看完能用上"
- **结论**：v2.0 不应继续堆 source，应聚焦"采回来的 N 条如何进入知识库"

**2. 知识的本质是连接**
- 单一 concept 没价值，concept A → B → C 的网络才是复利
- v1.7 设计的"事件-实体"模型是正确方向，但**没落地**
- **结论**：v2.0 必须让 `item_entities` 表活起来

**3. 复利的关键在"读旧写新"**
- 用户每天产出 200-500 条信息，但知识库增长 0 条/天 = 复利断裂
- 4,147 items 100% 有 lifecycle 但 0% 用 v2.0 5 阶段 = 全部 items 都用错的旧 3 阶段值
- **结论**：v2.0 必须让"自动入库"成为默认，而不是"手动标记"

**4. MCP 是放大器，不是替代品**
- v1.7 设计：MCP 写 = 外部 AI Agent 主动调
- 实际：用户很少主动打开 AI Agent 调 MCP（需要"触发时刻"）
- **结论**：v2.0 应让"自动触发"成为常态（如：catchup 完成后自动调 MCP 评分 + 入库；用户收藏时自动调 MCP 提炼 concept）

**5. 单人本地优先是约束也是护城河**
- 约束：不能假设有云 LLM、有 Redis、有多用户
- 护城河：99% 的 SaaS 抓取工具（Feedly、Inoreader、NewsBlur）都是云服务，**本地优先是差异化**
- **结论**：v2.0 任何设计都要问"无外部依赖能跑吗？"
...
**v1.7 没有 (1)(2)(3) 的自动化**，全靠手动。v2.0 必须打通。
...
| 维度 | hotspot v2.0 | Horizon | Feedly | Notion AI | Obsidian + 插件 |
|---|---|---|---|---|---|
...
- **核心目标**：hotspot v2.0 从"采集导向"转向"复利导向"——让每天 200–500 条新信息自动沉淀、复用、产生复利，而非继续堆采集源。
- **关键决策**：保留 KL 五阶段 + 引入 5 自动化触发器（T1–T5）强制推进生命周期；引入 simhash 跨源去重；MCP 叙事从"支持 MCP"升级为"AI Agent 可读写你的本地知识库"。
- **依据**：v1.7 采集层已成熟（46 源/28 活跃），但 4,147 条 items 100% 有 lifecycle 但 0% 用 v2.0 5 阶段（实际 100% 用 v1.7 旧 3 阶段 `generate`/`signal`/`amplify:tagged`）、知识库日增量≈0——瓶颈在消费端，不在采集端。
...
| 预期影响 | 知识库日增量 ≈0 → ≥10/天；新信息复用率 ≈0% → ≥30%；v2.0 5 阶段 lifecycle 应用率 0% → migration 046 迁移后 100% |
...
让每天 200–500 条新信息自动经过降噪、去重、评分、关联、结构化、发布，沉淀进本地知识库。直接对标本北极星"知识库日增量 ≥10/天"与"新信息复用率 ≥30%"，解决 100% items 用 v1.7 旧 3 阶段（应迁 v2.0 5 阶段）的核心断点。
...
**核心用户画像**: hotspot v2.0 的核心用户是"以知识为生产力的单机安全从业者"——参考品牌 SecNews，典型画像为拥有约 10 年攻防演练与咨询经验的安全老兵，关注 AI 安全、网络安全、安全从业转型与安全规划建设运营。工作环境高度本地化：单机作业、无团队协作假设、强烈的数据隐私诉求，这与 v1.7"本地优先（.md + SQLite，Obsidian 可直读）"的架构天然契合。核心诉求经历了一次本质迁移——从"把 46 个源的信息采回来"转向"采回来的东西每天能自动沉淀、复用、并产生复利"。其真实工作流高度规律：早上看热点 → 收藏 → 周末/晚间手动整理写 SOUL、提炼 concept → 写代码时检索触发 Todo/Codegarden；这一链路目前在每个"→"处都依赖人工，是 v2.0 必须自动化的断点。

**核心痛点与未被满足需求**: 草案数据揭示三个结构性断点：① 复利断裂：4,147 个 items 100% 有 lifecycle 但 0% 用 v2.0 5 阶段（100% 用 v1.7 旧 3 阶段值 `generate`/`signal`/`amplify:tagged`）；② 知识库日增量≈0：每天 200–500 条新信息进来，能进入 knowledge/items/ 的几乎为零；③ 手动整理负担重：周末/晚间手动整理、写 SOUL、提炼 concept 全靠人，MCP 13 tool 中 4 个低频（缺触发时刻）。未被满足需求：用户要的不是更多源，而是"采回来的 N 条如何自动进入知识库"以及"知识的本质是连接"。

**对 v2.0 关键设计的用户价值验证**:

- KL 五阶段 + 5 自动化触发器 + migration 046 命名迁移：直接命中断点。T1 解决"卡在 raw"、T2 解决"连接靠手建"、T3/T4 把结构化+发布默认化、migration 046 把 v1.7 旧 3 阶段值统一到 v2.0 5 阶段。100% items 用 v1.7 旧 3 阶段不是阶段本身的问题，而是命名未统一 + 没有触发器——验证保留 5 阶段。
...
1. v2.0 优先级压在"知识复利自动闭环+跨源去重+AI评分 MCP 工具"，而非新增抓取源。
...
| 应用 v2.0 5 阶段 lifecycle items | 0/4,147 = 0% | 100%（迁移后）| ❌ 老数据全用 v1.7 旧 3 阶段 |
| lifecycle 实际取值分布 | generate: 74 / signal: 4,070 / amplify:tagged: 3 | 5 阶段全替换 | ❌ 与 v2.0 5 阶段 100% 不一致，需 migration 046 |
| 采集源 | 46（28 活跃） | — | ✅ 成熟 |
...
> **关键缺口分析**: 采集层已成熟（46源/28活跃+全套验证防抖），但 4,147 条 100% 有 lifecycle 而 0% 用 v2.0 5 阶段（实际 100% 用 v1.7 旧 3 阶段值）。问题重心已从"采得回"转移到"采回来之后怎么用"。日增量≈0、复用率≈0 是这一断裂的量化结果。这构成 v2.0 从采集导向转向复利导向的唯一决策依据。
...
1. **为何不再堆 source**: 采集已成熟，但 0% 用 v2.0 5 阶段、复利闭环未建立、日增量≈0，新增源边际收益趋零，瓶颈在消费端。
...
6. **不默认自动建立 knowledge_links（无需确认）**——v2.0 需 AI Agent 经 link_items 确认，错误链接伤害信任，v2.1 再自动化。
7. **不默认把 0–10 热点评分自动应用**——v2.0 评分先存待人工确认，用户对 AI 评分信任需累积，v2.1 再自动应用。
...
> hotspot v2.0 为常驻本地优先服务，采用 46 天密集交付（设计目标，非承诺计划）。下表以"阶段"替代日历季度；若映射季度可视为相对时间（M1≈D1–18、M2≈D19–28、M3≈D29–34、M4≈D35–46），但实际以交付里程碑为准，不绑定日历。Phase 8–16 的具体周期见 B.10。
...
> **hotspot v2.0 = 单人本地"信息→知识→复利"中转站，46 源自动降噪、AI 评分、概念提炼、跨库连接，让知识库每天自动增长，配套 Codegarden + Security Graph 双子系统。**
...
> **总览**: v2.0 新增 6 张表（4 张核心 + 2 张可选/规划）。4 张核心：`content_fingerprints` / `ai_scores` / `item_entities` / `knowledge_links`；2 张扩展：`knowledge_chunks`（段落级引用）/ `planning_actions` + `planning_action_log`（规划引导）。
...
-- 5. 段落级引用（v2.0 必做，T3 触发器写，详见 B.4.5）
...
| `knowledge_items.lifecycle` 5 阶段 | **完全保留**（v1.7 设计，v2.0 加 5 触发器强制推进）|
| `knowledge_items.chunks` YAML + SQLite | **v2.0 引入**（v1.7 设计，吸收 SAG 思想，详见 B.4.5）|
| `knowledge_items.attention_score` | **v2.0 新增字段**（基于 reading_states 聚合，详见 B.4.6）|
| `knowledge_chunks` SQLite 表 | **v2.0 新增**（chunk 索引，详见 B.4.5.3）|
...
| Local LLM 配置文件 | **v2.0 新增** `config/llm.yaml`（详见 B.13）|

### B.4.4 知识库 .md 文件格式（v2.0 简化）
...
chunks: 4                               # v2.0 新增：chunk 总数（详见 B.4.5）
attention_score: 0.78                   # v2.0 新增：注意力评分（详见 B.4.6）
---
...
### B.4.5 chunks 字段（v2.0 引入，吸收 SAG 思想）

> **v2.0 引入 chunks**：v1.7 设计了但未实施，v2.0 决定引入。**为什么引入**：chunks 是 SOTA 检索架构的基础单元（参考 SAG），支持段落级精确引用、增量摘要、用户可点击跳转原文位置。
...
#### B.4.5.3 SQLite 缓存（v2.0 新增表）

```sql
-- migration 046_v2.0_chunks.sql
...
> **v2.0 引入注意力评分**：基于 reading_states + 浏览时长 + 滚动深度 + 标签匹配 + 用户标记，构建 item 级别的"注意力分数"。
...
**hotspot v2.0 的 6 步管线**（保留 Horizon 设计 + 改造为本地优先）:
```
...
```python
# backend/collectors/base.py v2.0
...
|---|---|---|---|
| HackerNews | `hn` | `story` / `comment` | v2.0 新增（借鉴 Horizon）|
| Reddit | `reddit` | `post` / `comment` | v2.0 新增 |
| RSS | `rss` | `item` | 已有 8 个，v2.0 标准化 |
| GitHub | `gh` | `release` / `trending` / `repo` | 已有 |
| OpenBB | `openbb` | `news` | v2.0 新增（金融）|
| Telegram | `tg` | `message` | v2.1 推迟（需 playwright + 反爬强；与 Twitter/X 同等复杂度）|
| Twitter/X | `x` | `tweet` | v2.1 推迟（反爬强）|
| GDELT | `gdelt` | `event` | v2.0 新增（安全事件）|
| OssInsight | `oss` | `repo` | v2.0 新增（开源洞察）|

**v2.0 新增 6 个 collector，借鉴 Horizon scrapers 实现**

---

## B.6 知识层（KL 5 阶段 + 5 自动化触发器 + 资讯收藏聚合视图）

> **本节是 v2.0 PRD 的核心**。v1.7 设计了 KL 5 阶段但**没有触发器** + 100% items 实际用 v1.7 旧 3 阶段值（命名不一致）。v2.0 通过 5 个自动化触发器强制推进 lifecycle + migration 046 强制命名迁移，配合认知链路 6+1 环节实现知识复利。
...
- 100% items 用 v1.7 旧 3 阶段值（应迁 v2.0 5 阶段）不是因为阶段太多，而是因为**命名未统一 + 没有触发器**——这是 v1.7 的真正问题

### B.6.2 5 个自动化触发器（v2.0 核心创新）
...
def trigger_t3_link_to_structure(hotspot_id: str):
    ...
    # 4. 写摘要文件 + chunk 摘要（v2.0 合并到 T3，由 LLMService 批量生成）
...
```python
# backend/scheduler/jobs.py v2.0
...
| sync_bundle | 跨端同步时新增 `imported_aggregator_cache` 缓存表（v2.0 推迟到 v2.1）|
...
## B.7 MCP 集成（v2.0：读独立 + 写副作用 + 失败恢复 + 规划引导）

> **v2.0 核心变更**: v1.7.6 仅有"同步直返 + 零状态"的简单 tool 列表。v2.0 引入**"读独立 + 写副作用"混合模式**（基于"服务之间数据/流程是否关联，关联则联动"原则），并为每个写 tool 设计**失败恢复动作**。

### B.7.1 MCP 13 tool 清单 + 模式标记（v2.0 组成优化，数量不变）
...
| **写 - 新增（4 个，副作用）** | `[B]` | `score_item` | AI 评分 | **关联**: ai_scores / ai_score ≥ 7 触发 T1 推进；score < 5 自动 archive | 🆕 v2.0 |
| | `[B]` | `enrich_concept` | 概念背景补全 | **关联**: concepts/{name}.md / item_entities 候选集 | 🆕 v2.0 |
| | `[B]` | `link_items` | 知识关联 | **关联**: knowledge_links / concept graph 增量更新 | 🆕 v2.0 |
| | `[B]` | `trigger_codegarden_drift` | 项目技术栈漂移检测 | **关联**: cg_projects.tech_stack 评估 / alerts | 🆕 v2.0 |
| **删除（v2.0 移除 4 个）** | - | ~~`trigger_extract_tags`~~ | 自动标签提取 | 改为 T1 触发器自动（Phase 10）| 移除 |
| | - | ~~`trigger_cubox_sync`~~ | 同步 | 保留为本地 cron job `cubox_sync` | 移除 |
| | - | ~~`create_alert_rule`~~ | 告警 | 推迟 v2.1（v2.0 内置 3 类基础规则）| 移除 |
...
### B.7.5 规划引导（v2.0 新增，13 个 tool 之外的常驻能力）
...
#### B.7.5.2.1 SQLite 表设计（v2.0 新增）

```sql
-- migration 047_v2.0_planning_actions.sql
...
## B.9 不做清单（v2.0 明确放弃，更保守）

> **v2.0 不做清单原则**: 优先保留 v1.7 优秀设计，仅明确放弃"长期未实施且无明显 KPI 价值"的设计；模糊价值的项目**保留为设计**而非直接砍。

| 不做 | 原因 | 处置 |
|---|---|---|
| 告警系统 M6 | ~~优先级低于复利~~ | **v2.0 实施 3 类基础规则**（tech_stack/CVE/标讯），v2.1 扩展 → 见 Phase 11.5-11.8 |
| 6 种认知模式 → 3 种 | ~~单人界面模式少~~ | **保留 6 种设计，v2.0 全部实施**（Hybrid AI + chunks + attention_score 支撑整理/复习模式）|
| 快速捕捉 Quick Capture | 浏览器插件开发成本高 | 推迟到 v2.2 |
| 注意力热图 | 数据稀疏（reading_states 数据不足）| **v2.0 引入**（reading_states + LLM 摘要 + 30 天数据即可使用，详见 B.4.6）|
| 决策日志 | 概念模糊 | 永久不做（已确认）|
| 离线间隔摘要 | ~~同质化功能~~ | **v2.0 实施**：catchup 失败恢复后自动生成补丁摘要 |
...
| KV 缓存层 | 评估后保留为可选，v2.0 移除 | 永久不做（已确认）|
| `chunks` YAML 字段 | 本地无 LLM | **v2.0 引入**（Hybrid AI 模式下本地 LLM 可用，详见 B.4.5）|
...
| **整理模式（Outbox）** | v1.7 设计未实施 | **v2.0 实施**（reading_states 累积 + LLM 摘要支持，详见 Phase 12）|
| **复习模式（SM-2）** | v1.7 设计未实施 | **v2.0 实施**（sm2_reviews 累积，详见 Phase 12）|
...
> **v2.0 全部实施 6 种认知模式**：基于 reading_states 数据积累 + LLM 摘要支持，整理/复习模式可落地。

| 模式 | 触发条件 | 界面 | 核心操作 | v2.0 状态 | Phase |
|------|---------|------|---------|----------|-------|
...
| **sync_bundle** | reading_states + annotations + sm2_reviews 跨端同步（v2.0 简化为 4 表）|
...
### 关键 endpoint（v2.0 必保留）

| Method | Path | 用途 | v2.0 变化 |
|--------|------|------|----------|
...
| GET | `/api/knowledge/items/{id}/triggers` | **v2.0 新增** 5 触发器状态 | 🆕 |
| POST | `/api/knowledge/items/{id}/transition` | **v2.0 新增** 手动触发 T1-T5 | 🆕 |
| GET | `/api/knowledge/imported` | **v2.0 新增** 资讯收藏聚合视图（favorites ∪ knowledge_items 去重）| 🆕 |
| GET | `/api/knowledge/compounding` | **v2.0 新增** 复利仪表盘数据 | 🆕 |
| GET | `/api/alerts` | **v2.0 新增** 告警列表 | 🆕 |
...
| GET | `/api/metrics/kl` | **v2.0 新增** 触发器指标 | 🆕 |
...
> **v2.0 新增 9 个 job**：T1/T2/T3/T4 四个 IntervalTrigger + T5 主动触发。

### APScheduler 任务清单（v2.0 = 30 jobs）

| Job ID | 名称 | 触发器 | 周期 | v2.0 状态 |
|--------|------|--------|------|----------|
...
**总计 24 个 job**（v1.7 15 个 + v2.0 新增 9 个：4 触发器（kl_trigger_t1/t2/t3/t4）+ dead_letter_retry + daily_digest + compounding_metrics + attention_aggregate + planning_action_check）

> **实施灵活度说明**: 13 tool 是设计目标，**实施顺序可根据用户实际需求调整**。Phase 8-16 表的周期是"设计目标"，不是"固定计划"。hotspot 是常驻服务，用户可在任意 Phase 启动 v2.0 部分能力，剩余能力渐进式补齐。
...
> **v2.0 新增 4 模式 UI + 复利仪表盘 + 触发器状态可视化**。
...
| `/knowledge/imported` | `KnowledgeFavoritesView` | **v2.0 新增** 资讯收藏聚合视图（5 类型筛选 + 名称搜索 + 时间范围 + 分页）| 🆕 Phase 8 |
| `/knowledge/compounding` | `KnowledgeCompoundingDashboard` | **v2.0 新增** 复利仪表盘 | 🆕 Phase 12 |
| `/knowledge/briefing` | `BriefingMode` | **v2.0 新增** 简报模式 | 🆕 Phase 12 |
| `/knowledge/scan` | `ScanMode` | **v2.0 新增** 扫描模式 | 🆕 Phase 12 |
| `/knowledge/deep` | `DeepReadMode` | **v2.0 新增** 深度阅读 | 🆕 Phase 12 |
| `/alerts` | `AlertCenter` | **v2.0 新增** 告警中心 | 🆕 Phase 11 |
...
| `KnowledgeFavoritesView` | `components/knowledge/` | **v2.0 新增** 资讯收藏聚合视图：5 类型筛选 + 名称搜索 + 时间范围 + 分页 + origin 分布 |
...
> **v2.0 同步表扩展**: 在 v1.7 4 表核心（reading_states + favorites + tags + sm2_reviews）基础上，**新增 4 表**（lifecycle / knowledge_links / ai_scores / item_entities）共 **8 表同步**。
...
**v2.0 新增 4 表同步**：lifecycle / knowledge_links / ai_scores / item_entities（设备本地状态优先）
...
> **与 B.10.12 一致**：10 个 Phase（v2.0 的 9 个 + 预置 v1.9 标准化 1 个）总计 ~46 天。Phase 9 为 v1.9 已完成基线，详见 phase9_changelog.md。Phase 16/17 详见 B.10.10 / B.10.11。

| Phase | 名称 | 周期 | 模块 | 5 触发器对应 | 状态 |
|---|---|---|---|---|---|
| **8** | 复利基础设施 + 资讯收藏聚合 | ~6 天 | simhash 去重 + ai_scores/item_entities/knowledge_links 表 + 4 个新 MCP tool + **资讯收藏聚合视图（B.6.7）**| T1 基础设施 | 🆕 v2.0 |
| **9** | **资讯抓取流程标准化** | **~4 天** | **断点续传（catchup_checkpoints）+ 4 类数据完整性验证（collect_validations）+ 结构化事件日志** | **抓取可靠性** | **✅ v1.9 已完成** |
| **10** | T1/T2 触发器实施 | ~4 天 | T1 (raw→refine) + T2 (refine→link) + 状态机引擎 | T1, T2 | 🆕 v2.0 |
| **11** | 抓取层现代化 | ~5 天 | BackendSession + 可读 ID + trafilatura + 6 个新 collector | T1 数据源 | 🆕 v2.0 |
| **12** | T3/T4/T5 触发器 + 告警系统 | ~6 天 | T3/T4/T5 + 3 类基础告警规则（tech_stack/CVE/标讯） | T3, T4, T5 | 🆕 v2.0 |
| **13** | 复利可视化 + 4/6 模式 + 规划引导 | ~4 天 | KnowledgeCompoundingDashboard + 简报/扫描/深度/告警 4 模式 UI + KnowledgePlanningPanel | T4 验证 | 🆕 v2.0 |
| **14** | 子系统联动 | ~3 天 | tech_stack_drift + cve_knowledge_sync + 跨域 entity 命名空间 | T2 联动 | 🆕 v2.0 |
| **15** | 清理 + 文档 + 迁移 | ~5 天 | 删 kv_cache / /api/agent / 4 MCP tool / 迁移指南 / 用户文档 | T5 兜底 | 🆕 v2.0 |
| **16** | **Hybrid AI（Crawl4ai + 本地 LLM + 外部 Agent 并存）** | **~6 天** | **LLMService 多 provider + Crawl4ai 4 源迁移 + 5 种降级场景**（详见 B.10.10）| 性能提升 | 🆕 v2.0 |
| **17** | **Chunks + Attention Heatmap + 6 模式完整** | **~7 天** | **chunks 表 + 热图 + 整理/复习模式**（详见 B.10.11）| 复利闭环 | 🆕 v2.0 |

**总预估**: ~46 天（v1.7 的 28 天 → v2.0 46 天，**+64%**；5 阶段 + 5 触发器 + 告警恢复 + 4 模式 UI + Hybrid AI + chunks/attention + 6 模式完整 + 规划引导是核心增量）

> **详细周期分配**: 6 + 4 + 5 + 6 + 4 + 3 + 5 + 6 + 7 = 46 天（v2.0 Phase 8 + 10–17，不包含 v1.9 Phase 9 已完成基线；与 B.10.12 总览一致）。

### B.10.2 Phase 8：复利基础设施

| 任务 | 详情 | 验收 |
|---|---|---|
| **8.1 数据迁移** | `migration 043_v2.0_fingerprints_scores.sql`：新增 `content_fingerprints / ai_scores / item_entities / knowledge_links` 4 表 | schema 校验通过 |
...
| 任务 | 详情 |
|---|---|
| **15.1 删 kv_cache** | `migration 045_v2.0_drop_kv_cache.sql` |
...
| **15.4 写 v2.0 迁移指南** | `docs/v1_to_v2_migration.md`：含 5 阶段映射 + 触发器启用步骤 |
| **15.5 写 v2.0 用户文档** | `docs/hotspot_v2_user_guide.md`：5 触发器说明 + 4 模式使用 |
| **15.6 更新 CHANGELOG** | `docs/CHANGELOG.md`：v2.0 新增功能 + 破坏性变更 |
| **15.7 更新 README** | 同步到 v2.0 状态（5 子系统、13 MCP tool、5 阶段）|

### B.10.10 Phase 16：Hybrid AI（Crawl4ai + 本地 LLM + 外部 Agent 并存）

> **v2.0 重大设计变更**：v1.7 Option A 完全依赖外部 Agent，v2.0 引入 Hybrid AI 让 hotspot 具备可选的本地 AI 能力。详见 B.13 完整设计。
...
> **v2.0 收尾 Phase**：完成 v1.7 优秀设计的全部实施。详见 B.4.5 / B.4.6 / B.9.5。

| 任务 | 详情 | 验证 |
|---|---|---|
| **17.1 chunks 字段迁移** | `migration 046_v2.0_chunks.sql` + `knowledge_chunks` 表 | schema 校验通过 |
...
### B.10.12 Phase 总览（v2.0 = 10 个 Phase，~46 天）

| Phase | 名称 | 周期 | 关键交付 | 状态 |
|---|---|---|---|---|
| **8** | 复利基础设施 + 资讯收藏聚合 | ~6 天 | simhash + 4 表 + 4 MCP tool + **KnowledgeFavoritesView（B.6.7）**| 🆕 v2.0 |
| **9** | 资讯抓取流程标准化 | ~4 天 | 断点续传 + 4 类验证 + 结构化日志 | ✅ v1.9 已完成 |
| **10** | T1/T2 触发器 | ~4 天 | 状态机 + 60s/120s 调度 | 🆕 v2.0 |
| **11** | 抓取层现代化 | ~5 天 | BackendSession + 6 个新 collector | 🆕 v2.0 |
| **12** | T3/T4/T5 + 告警 | ~6 天 | 状态机完成 + 3 类告警规则 | 🆕 v2.0 |
| **13** | 复利可视化 + 4/6 模式 + 规划引导 | ~4 天 | 仪表盘 + 简报/扫描/深度/告警 + PlanningPanel | 🆕 v2.0 |
| **14** | 子系统联动 | ~3 天 | tech_stack_drift + cve 同步 | 🆕 v2.0 |
| **15** | 清理 + 文档 + 迁移 | ~5 天 | 删 4 项 + 迁移指南 | 🆕 v2.0 |
| **16** | **Hybrid AI** | **~6 天** | **LLMService + Crawl4ai + 4 源迁移** | 🆕 v2.0 |
| **17** | **Chunks + Attention + 6 模式完整** | **~7 天** | **chunks 表 + 热图 + 整理/复习模式** | 🆕 v2.0 |
| **总预估** | | **~46 天** | 6+4+5+6+4+3+5+6+7（v2.0 Phase 8 + 10–17，不含 v1.9 Phase 9）| |

**总变化**: v1.7 28 天 → v2.0 46 天，**+64%**（5 阶段 + 5 触发器 + 告警 + 4 模式 + Hybrid AI + chunks/attention + 6 模式完整 + 规划引导）

> **实施灵活度（重要）**: hotspot 是**常驻服务**而非一次性项目。上表 10 个 Phase 是**设计目标**，不是固定计划。用户可在任意 Phase 启动 v2.0 部分能力，剩余能力渐进式补齐：
...
```
        ╱╲
       ╱  ╲         E2E（端到端）
      ╱ 5% ╲         - Playwright/Cypress（v2.0 不实施，前端手动）
     ╱──────╲
    ╱        ╲       集成测试（API + DB + 调度器）
   ╱   25%    ╲      - test_*.py e2e 流程
  ╱────────────╲
...
| e2e | pytest + requests | 24 job 启动验证 | test_kl_triggers_e2e.py（v2.0 新增）|
...
## B.11.6 迁移策略（v1.7 → v2.0）

> **保留 v1.7 迁移原则**：数据零丢失、平滑过渡、可回滚。

### 5 阶段映射

| v1.7 lifecycle | v2.0 lifecycle | 迁移 |
|----------------|---------------|------|
...
1. **migration 043_v2.0_fingerprints_scores.sql**：
   - 新增 4 表：`content_fingerprints / ai_scores / item_entities / knowledge_links`
   - 加 6 个索引（按查询模式优化）

2. **migration 044_v2.0_kl_rename.sql**（可选，5 阶段已用 `kl:` 前缀）：
...
3. **migration 045_v2.0_drop_kv_cache.sql**：
...
**问题**: 生产 `knowledge_items.lifecycle` 字段 100% 有值，但全部是 v1.7 旧 3 阶段（`generate` / `signal` / `amplify:tagged`），与 v2.0 设计 5 阶段（`kl:raw/refine/link/structure/publish`）不一致。

**映射规则**:
- `signal`（未精炼）→ `kl:raw`
- `amplify:tagged`（已打 tag 待关联）→ `kl:refine`
- `generate`（已生成摘要）→ `kl:structure`（v2.0 升级到 kl:structure 等同 generate 的语义，并加 T3 触发 chunks）
...
- **前端**：v1.7 前端可继续用 v2.0 后端（忽略新字段）
- **后端**：v2.0 后端不能向下兼容 v1.7 前端（部分 UI 缺失）
- **数据库**：v1.7 DB 可被 v2.0 读取（migration 自动）
- **MCP**：v1.7.6 agent 调 v2.0 缺失 tool 会报错，建议升级
...
| 2 | AI 评分波动（同一新闻两次评分差大）| 高 | 中 | 存多版本评分；用最新 + 来源标识；v2.0 不自动应用，v2.1 引入置信度 |
| 3 | 自动入库导致知识库噪声 | 中 | 中 | 阈值 ≥ 7 才入；v2.0 不自动应用 knowledge_links；v2.1 引入"已读自动归档" |
| 4 | 6 个新 collector 反爬失败 | 中 | 低 | 借鉴 Horizon 失败模式；v2.0 先实现 3 个（hn/rss/openbb），其余 3 个 v2.1 |
| 5 | trafilatura 引入新依赖 | 低 | 低 | 作为 optional，pip install 时可选 |
| 6 | 可读 ID 迁移复杂 | 中 | 中 | 保留 hash ID 作为 alias；v2.0 双写，v2.1 切读路径 |
...
| 10 | 删 /api/agent 路由破坏旧 agent | 低 | 中 | v2.0 保留 deprecated 1 个 minor 版本，v2.1 完全删 |
...
| 15 | attention_score 数据稀疏（v1.7 99.4% items 无 reading_states）| 高 | 中 | v2.0 启动时 backfill：从 SQLite history 推断初始 score；3 个月内新数据为主，旧 items 缓慢覆盖 |
| 16 | Crawl4ai / Playwright 代理配置与 proxy_config.json 不一致 | 中 | 中 | 统一从 `proxy_config.json` 读代理（见 B.13.5.2 v2.0 改造），删除 crawl_config.yaml 的 proxy_pool 硬编码；Playwright 通过 BackendSession 注入 |
...
> **v2.0 重大设计变更**：v1.7 Option A 完全依赖外部 AI Agent；v2.0 引入 **Hybrid AI**：hotspot 可选配置本地 LLM + Crawl4ai 高阶抓取，与外部 Agent 并存，发挥各自优势。
...
| 任务 | v1.7（外部 Agent）| v2.0 Hybrid（默认）| 降级（无 LLM 配置）|
|------|------------------|------------------|------------------|
...
```python
# backend/services/llm_service.py (v2.0 新增)
...
> **v1.7 PRD 提及 Crawl4ai 但未落地**，v2.0 正式实施。Crawl4ai 是开源 LLM-friendly web crawler，专为 AI 数据提取设计。
...
#### B.13.5.2 Crawl4ai 配置（v2.0 统一 proxy + Playwright 同步）

> **v2.0 改造原则**: Crawl4ai 与 Playwright 都走统一代理配置，删除 `crawl_config.yaml` 中 proxy_pool 硬编码，避免双源配置漂移。
...
```yaml
# backend/collectors/crawl_config.yaml (v2.0 改造后)
crawl4ai:
  enabled: true
  ...
  # ❌ 删除 proxy_pool 硬编码（v2.0 统一从 proxy_config.json 注入）
  # anti_bot:
...
```python
# backend/parsers/crawl4ai_parser.py v2.0
...
```python
# backend/services/proxy_pool.py (v2.0 新增)
class ProxyPool:
...
```sql
-- migration 048_v2.0_proxy_health.sql
...
```python
# backend/services/proxy_pool.py v2.0
...
| **6 种认知模式** | **设计完整，5 个未实施** | **v2.0 全部实施 6 种** | Hybrid AI + chunks + attention_score 支撑整理/复习模式 |
| **告警系统 M6** | **设计完整，未实施** | **实施 3 类基础规则**（tech_stack/CVE/标讯）| 用户明确要求恢复 |
| **离线间隔摘要** | **设计** | **v2.0 实施** | catchup 失败恢复后自动生成 |
| 快速捕捉 | 浏览器插件 | 推迟 v2.2 | 开发成本高 |
| **注意力热图** | **设计** | **v2.0 引入**（基于 reading_states + LLM）| 数据足够 + LLM 摘要支持 |
| **chunks 字段** | **设计未落地** | **v2.0 引入**（Hybrid AI 模式）| Local LLM 兜底 + 段落级精确引用 |
| 决策日志 | 设计 | 不做 | 概念模糊 |
...
| **LLM 配置文件** | **无** | **v2.0 新增 `config/llm.yaml`** | 多 provider + 降级 + 成本控制 |
| **Crawl4ai 高阶抓取** | **PRD 提及未实施** | **v2.0 正式实施，迁移 4 源** | SPA/反爬强站必备 |
...
| 多语言日报 | 无 | v2.0 不做 | 单人 ZH 优先 |
| **5 触发器** | **无（v1.7 100% items 用 v1.7 旧 3 阶段值，未迁 v2.0 5 阶段）** | **T1-T5 自动化触发器 + migration 046** | v2.0 核心创新 |
| **状态机引擎** | **无** | **KLStateMachine + 不变量检查** | 复利基础设施 |
| **复利机制** | **设计无落地** | **T1-T5 + 复利仪表盘 + 关联率 ≥ 30%** | 核心 |
| **chunks 元数据** | **设计未落地** | **v2.0 引入 chunks + chunks_meta + knowledge_chunks 表** | 段落级精确引用 |
| **attention_score** | **无** | **v2.0 新增 5 维度加权 + AttentionHeatmap** | 注意力热图 |
| 三大子系统协同 | 弱 | 强联动 (tech_stack_drift / cve sync) | 差异化 |
| **资讯收藏聚合视图** | **无（favorites + knowledge_items 分散）** | **v2.0 新增 `/api/knowledge/imported` + `/knowledge/imported` 页面（5 类型 + 名称 + 时间 + 分页）**| B.6.7 |
...
| reddit.py | ✅ 借鉴 + 简化 | 砍 OAuth（v2.0 用 public JSON）|
...
|---|-----------|---------|---------|---------|
| 1 | **AI 评分默认自动应用还是存待确认？** B.9 建议 v2.0 先存待确认、v2.1 再自动，需确认用户信任阈值与过渡节奏 | B.0.8 #7、B.0.7 P0-5 | **v2.0 评分先存待确认**（与 P0-5 评分工具解耦；M1 不阻塞） | M1 末（首轮发布） |
| 2 | **自动建立 knowledge_links 是否需要用户确认？** v2.0 设计为 Agent 经 link_items 确认；是否引入"高置信度自动 + 低置信度待确认"混合策略待定 | B.0.7 P0-4、B.0.8 #6 | **保持"Agent 确认"模式**，v2.1 引入置信度分层 | M1 末 |
...
| 5 | **单人天花板付费验证节奏？** 竞析建议先用 1–2 个垂直场景（安全研究 / 个人技术雷达）验证单人付费意愿，再决定是否开放轻量协作——验证指标与时间点待定 | B.0.5 行动建议 #5、B.0.8 #1 | **v2.0 不开放协作**；M4 后开 1 个垂直场景试点 | v2.0 之后 |
...
| 7 | **评分阈值（默认 7.0）与发布阈值（默认 8.0）是否开放用户配置？** 安全从业者 vs 轻量浏览者对噪声容忍度不同，是否需要分群阈值？ | B.0.6 北极星指标、B.0.7 P0-5 | **v2.0 内置默认阈值 + 高级配置可调**（Settings 面板） | M1 末 |
...
| 9 | **双子系统联动（Codegarden / Security Graph）是否纳入 v2.0 付费验证场景？** 还是先作为免费纵深能力提供、付费验证聚焦知识复利本身？ | B.0.2 目标③、B.0.7 P1-9 | **v2.0 保持免费纵深能力**；付费验证仅聚焦复利本身 | v2.0 之后 |
| 10 | **资讯收藏聚合视图的 5 类数据源是否覆盖全部高频场景？** 是否需要补充 Newsletter / YouTube 等，还是严格限制在现有 46 源内的聚合 | B.0.7 P1-3、B.6.7 | **v2.0 严格限制 5 类（favorites / cubox / bookmark / secnews_archive / secnews）**；Newsletter/YouTube 推迟 v2.1 | M1 末 |

> **处置原则**: M1 末（首轮可发布）前需对 #1 / #2 / #7 / #8 给出明确答案；#3 / #4 / #6 在 M3-M4 切档前定调；#5 / #9 / #10 推迟到 v2.0 之后。每次评审会议同步更新本附录（标记已确认 / 已推迟 / 仍待确认）。
...
## 附录 E · v2.0 PRD 实测对账与修复记录（2026-07-27 leader skill 产出）
...
2. **B.11.6 迁移策略完全缺 lifecycle 5 阶段数据迁移方案**——4,147 items 等着 v2.0 上线后跑迁移 SQL。Patch 3 已给出可直接用的 SQL。
...
> **触发**: Round 1 patch 后，[v2_prd_review.md](../v2_prd_review.md) 偏差表只覆盖了 A.1.1 第 3 行，但**同一错误数据 "25/4127=0.6%" / "99.4% 卡在 kl:raw" 在 PRD 主体中重复 15+ 次**（L90/L147/L166/L281/L367/L376/L385/L407/L411/L475/L476/L483/L498/L499/L1171/L1187/L1967/L2026/L3755）。Round 2 把这 15+ 处全部统一为"4,147 items 100% 有 lifecycle 但 0% 用 v2.0 5 阶段（实际 100% 用 v1.7 旧 3 阶段值）"的新叙事，**同步调整 5 触发器论证逻辑**（从"99.4% 卡 raw"改为"100% 用错阶段 + 命名未统一 + 无触发器"）。
...
| **7.9** | B.0.4 对 v2.0 关键设计（L411）| ✅ | 1 行替换 + 论证逻辑加 migration 046 |
...
## 附录 F · v2.0 PRD 修复任务书归档（2026-07-27，已完成）

> **来源**: [`/Users/duke/.trae/documents/review_goal.md`](../../Users/duke/.trae/documents/review_goal.md)（leader skill 任务书，160 行 / 8KB）
> **方法**: 把 v2.0 PRD 复盘发现的 6 个 P0/P1 偏差一次性修掉，让 PRD 从"草案"升到"可贴墙拍板"
> **性质**: 执行型任务书归档；本附录是执行层归档，与 E.4 patch 落地表一一对应。

### F.1 任务目标

**目标**: 把 v2.0 PRD 复盘发现的 **6 个 P0/P1 偏差**（A.1.1 第 3 行失实 + B.3 架构图 4 数字过时 + 5 阶段 lifecycle 数据迁移方案缺失 + 端点分工不明确 + 反向验证测试缺失 + 13 tool 模式标记缺失）一次性修掉。

**让步顺序**: 数据准确 > 数字齐全 > 不破坏现有内容。

### F.2 任务界限

- **只允许改**: `docs/hotspot_v2.0_PRD.md`（6 patch 全部落到这 1 个文件）
...
**Round 2 关键设计变更**:
- 叙事基线: "25/4127=0.6% 没 lifecycle" → "4,147 items 100% 有 lifecycle 但 0% 用 v2.0 5 阶段（100% 用 v1.7 旧 3 阶段值）"
...
grep -nE "25/4127|25/4,127|4127 条仅 25|99\.4% items 卡|99\.4% 卡在 \`kl:raw\`|卡在 kl:raw" docs/hotspot_v2.0_PRD.md
# 实际: 4 行匹配，全部在 L3845/L3859/L3928/L4008 附录 E/F 内
```

### F.4 完成情况

- **硬指标 1**: `docs/hotspot_v2.0_PRD.md` 总行数变化（应用 5 patch 后 +50 行、新增附录 E/F +160 行）→ 已记录在验收报告
...
> **最后**: hotspot v2.0 的核心是**让知识库每天自动增长**（复利），通过 **5 阶段 + 5 触发器** 强制推进 lifecycle，配合 24 个 APScheduler job + 13 个 MCP tool + 6 种认知模式 + Hybrid AI（Crawl4ai + 可选本地 LLM + 外部 Agent）+ chunks 段落级引用 + attention_score 注意力热图，实现从「信息→知识→复利」的完整闭环。
>
