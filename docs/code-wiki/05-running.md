...
> **本文件是 [`../ARCHITECTURE.md`](../ARCHITECTURE.md) §一"系统总览"中运维层的细节展开** — 详列依赖/启动/测试/构建/CI 步骤, 接手会话必读。
...
- 启动后自动: 迁移 → 缓存预热 → 调度器 (5s 后首跑采集) → knowledge watchdog →
  自动追抓本周一 00:00 以来的资讯
- 健康检查: `GET /api/health` (含 scheduler 状态)
...
### 3.3 MCP stdio (外部 Agent 接入)

```bash
python -m backend.mcp_stdio_main
```

SSE 通道随主服务启动 (`/mcp/sse`, 受 `mcp` feature gate 控制)。
...
| 配置 | 位置 | 用途 |
|------|------|------|
| 环境变量 | 前缀 `HOTSPOT_` (`backend/config.py`) | host/port/采集间隔/watchdog/catchup/feature_* |
| Feature gates | `backend/config/feature_gates.toml` | 扩展开关单一来源; env `HOTSPOT_FEATURE_GATES` (JSON) 可覆盖 |
| LLM | `config/llm.yaml` + `backend/config/llm_schema.py` | provider / fallback_order |
...
| Agent | `config/agents.yaml` | agent 配置 |
...
.venv/bin/python3 -m py_compile backend/services/sync_merge.py    # 编译检查
...
python scripts/harness_analyze.py --format json
...
| Agent 资产 | `harness_analyze.py --check` (长 skill >500 行必须有 `references/`, baseline 豁免见 `scripts/harness_baseline.json`) |
...
| 端点 | 用途 |
|------|------|
| `GET /api/health` | 健康 + scheduler 状态 |
| `POST /api/refresh` | 手动触发一轮采集 |
| `POST /api/catchup` | 手动追抓 (per-source checkpoint 断点续抓) |
| `POST /api/maintenance/cleanup-quality-logs?days=2` | 质量日志归档 (防 quality_check_logs 膨胀) |
...
- 路由文件 ≤ 150 行; 服务层禁止 `import backend.api`; `api/__init__.py` lazy import
...
- 测试环境必须 `HOTSPOT_CATCHUP_ON_STARTUP=false` (或 conftest 处理), 否则启动即触发追抓
- `retirement_baseline.json` / `blacklist_scenarios.json` 等根目录 JSON 为退役/回归基线数据,
  勿随手删除
