...
- fastapi-mcp 把 13 个 FastAPI 路由自动暴露为 MCP tool
- 双 transport:
  - stdio (默认, 本地单进程, AI Agent 启动)
  - SSE / StreamableHTTP (HTTP, 跨网络/调试用)
- 启动时 idempotent seeding 13 个 tool 元数据到 mcp_tool_registry 表
- feature.mcp_server (默认 True) 控制是否挂载 MCP

关键决策
---------
1. **范围**: 全量 13 个 tool (5 读 + 8 写), 1:1 对应 FastAPI 路由
...
# ----------------------------------------------------------------------------
# 13 个 MCP tool 对应的 FastAPI operation_id
# ----------------------------------------------------------------------------
...
MCP_TOOL_OPERATION_IDS = [
    ...
    "trigger_extract_tags_api_extract_auto_post",                  # trigger_extract_tags (mcp_adapters.py)
    "trigger_cubox_sync_api_cubox_sync_post",                      # trigger_cubox_sync (mcp_adapters.py)
    "create_rule_api_alerts_rules_post",                           # create_alert_rule
    "mark_read_api_digests_read_put",                              # mark_digest_read
]
...
def build_mcp_server(app):
    """构建 FastApiMCP 实例 (按 13 tool 列表 include_operations)。
...
def mcp_tool_registry_seed() -> int:
    """启动时把 13 个 tool 元数据写入 mcp_tool_registry 表。

    幂等: 重启不会重复插入 (用 PRIMARY KEY name 保证)。
    Returns: 实际写入的条数 (首次启动 = 13, 后续 = 0)。
    """
...
def list_mcp_tools_from_db(enabled_only: bool = False) -> list[dict]:
    """从 mcp_tool_registry 表读 13 tool 元数据。"""
    from backend.repository.db import get_connection
